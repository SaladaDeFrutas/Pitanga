        ...
        $(document).ready(function () {
            $("input[name='assentos']").change(function () {
                ...
                /*  if($("input[name='assentos.coluna']").is(':checked')){
                 $("#assentos.fileira").html("<input type="hidden" name = "assentos.fileira" value="
                ${i}"/>");
                 } */
            });
        });
...
<h4 class=quantidadeRestante>
    ...
    Ingressos restantes: <span id="quantidadeRestanteId"><c:out
    value="${quantidadeRestante}"/> </span>
...
<div class=posicaoSala>
    <form action="finalizarCompra" method="post">
                    ...
                    <c:forEach var="j" begin="0" end="${sala.qntColunas}">
                        <td class=posicaoCadeira>
                            ...
                            <c:forEach items="${assentosInvalidos}" var="assentoInvalido">
                                <c:if
                                    test="${assentoInvalido.fileira == i && assentoInvalido.coluna == j}">
                                    <img name="cadeira" id="cadeira"
                                         src="resources/assento_invalido.gif">
                            ...
                            <c:forEach items="${assentosOcupados}" var="assentoOcupado">
                                <c:if
                                    test="${assentoOcupado.fileira == i && assentoOcupado.coluna == j}">
                                    <img name="cadeira" id="cadeira"
                                         src="resources/assento_indisponivel.gif">
                                    <c:set var="entrou" scope="page" value="${1}"/>
                            ...
                            <c:if test="${entrou == 0}">
                                <img name="cadeira" id="cadeira"
                                     src="resources/assento_disponivel.gif">
                                <br/>
        ...
        <button type="submit" name="idBotao" value="" class="btn btn-default"
                onClick="return checarQuantidadeIngresso()">Finalizar Compra
        </button>
    </form>
</div>
</body>
